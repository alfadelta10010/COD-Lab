...
n:.dword 0x1234567899876543, 0x2467461398675234
result:.dword 0, 0 

.text
la x5,n   # Load the address of 'n' into register x5
lw x10,0(x5) # Load the first 8-byte (64-bit) value from 'n' into register x10
lw x11,4(x5)  # Load the second 8-byte value from 'n' into register x11
lw x12,8(x5)  # Load the third 8-byte value from 'n' into register x12 (however, this value doesn't exist in 'n')
lw x13,12(x5) # Load the fourth 8-byte value from 'n' into register x13 (also does not exist in 'n', it should be out of bounds)
...
la x7,result     # Load the address of 'result' into register x7
